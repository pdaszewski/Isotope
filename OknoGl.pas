...
type
  TAOknoGl = class(TForm)
    ...
    btn_change_isotope: TSpeedButton;
    TabSheet3: TTabSheet;
    Chart1: TChart;
    Series1: TAreaSeries;
    Label13: TLabel;
    ...
    procedure btn_change_isotopeClick(Sender: TObject);
    procedure btn_create_chartClick(Sender: TObject);
...
begin
  ...
  ADOConnection1.ConnectionString := 'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=' +
    ExtractFilePath(Application.ExeName) + 'BazaIzotopow.mdb;Persist Security Info=False;';
...
begin
  ...
  Begin
    ...
    if td <> 0 then
    Begin
      if cbox_algorythm.Checked = True then
        ak := ap * (exp((-0.693 * tm) / td))
      else
        ak := (ap) / (power(2, tm / td));
    End
    else
...
procedure TAOknoGl.btn_change_isotopeClick(Sender: TObject);
Var
...
begin
  ...
  btn_create_chart.Visible := True;
  Edit5.Visible := True;
  SpinEdit2.Visible := True;
end;

procedure TAOknoGl.btn_create_chartClick(Sender: TObject);
Var
  i, ile: Integer;
